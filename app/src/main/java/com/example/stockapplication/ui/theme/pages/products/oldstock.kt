...
@Composable
fun OldStockScreen(navController: NavHostController) {
    Column(
        modifier = Modifier.fillMaxSize(),
        horizontalAlignment = Alignment.CenterHorizontally
    ) {
        Text(text = "Welcome to old stock screen", color = Color.Magenta, fontSize = 30.sp)
    }
}
@Preview
@Composable
fun OldStockScreenPreview() {
    StockApplicationTheme {
        OldStockScreen(rememberNavController())
    }
}